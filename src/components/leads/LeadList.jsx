        ...
        <div className="mb-6 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="text-sm border border-border text-subtle rounded-lg px-4 py-2 hover:bg-raised hover:text-ink transition-colors"
            >
          ...
          <div className="border border-border rounded-xl overflow-x-auto bg-card">
            <div className="min-w-[720px]">
              ...
              {filteredLeads.map((lead) => (
                ...
                >
                  <span className="col-span-3 text-sm text-ink font-medium truncate">{lead.fullName}</span>
                  <span className="col-span-2 text-sm text-subtle capitalize">{lead.source}</span>
                  <span className="col-span-2">
                    <span className={`inline-block text-xs px-2 py-0.5 rounded-full font-medium capitalize whitespace-nowrap ${STATUS_STYLES[lead.status] || 'bg-raised text-subtle'}`}>
                      {lead.status?.replace('_', ' ')}
                    </span>
                  </span>
                  <span className="col-span-2 text-sm text-subtle truncate">
                    {lead.assignedConsultantId || '—'}
                  </span>
                  <span className="col-span-2 text-sm text-subtle whitespace-nowrap">
